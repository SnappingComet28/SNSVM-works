...
'''
startfile(r"C:\Users\snsvm\Desktop\SNSVM-works-main\Kserver.py")
sp(3)
...
class MyApp(MDApp):
    ...
    def cancelreq(self):
        server.send("__NONE__".encode('utf-8'))
    def accpet(self,dt):
        try:
            r=server.recv(1024)
            if r:
                ...
                if re =="__AcCpEcT__":
                    ...
                    rec = th.Thread(target=self.recieve)
                    rec.start()
                    Clock.unschedule(self.accpet)
                else:
                    pass
        except:
            pass
    def request(self):
        self.change_screen("w")
        server.send("__ReQuEsT__".encode('utf-8'))
